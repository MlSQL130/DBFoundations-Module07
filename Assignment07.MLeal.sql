...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_MLeal')
	 Begin 
	  Alter Database [Assignment07DB_MLeal] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_MLeal;
	 End
	Create Database Assignment07DB_MLeal;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_MLeal;
...
--This version adds spaces after month, but provides same answer
--Select ProductName,
--	 DateName (mm,InventoryDate) + ',' + str(DatePart(yy,InventoryDate)) As InventoryDate,
...
--Create Select Statement--this adds too many spaces after the month
--Select 
----	ProductName, 
----	DateName (mm,InventoryDate) + ',' + str(DatePart(yy,InventoryDate)) As InventoryDate,
...
--2nd Select Statement-this version only creates one space after month
...
As
	...
	[CategoryName], 
	[InventoryDate] = Format (InventoryDate,'MMMM' + ',' + ' ' +'yyyy'), 
	[InventoryCountByCategorySum] =  Sum(Count)
	From dbo.vCategories As c Join dbo.vProducts As p
	On c.CategoryID = p.CategoryID
...
--	Over (Order By ProductName, Month(InventoryDate)),0)
--	From vProductInventories
	

--Select Statement to add function to set January Null counts to zero
--Select 
--	[ProductName], 
--	[InventoryDate], 
--	[InventoryCount],
--	[PreviousMonthCount] = IIF(Month(InventoryDate)= 1,0, IsNull (Lag(InventoryCount) 
--	Over (Order By ProductName, Month(InventoryDate)),0))
--	From vProductInventories
--	Order by 1;
...
Return
	...
	[CountVsPreviousCountKPI]
	From dbo.vProductInventoriesWithPreviousMonthCountsWithKPIs
	Where (CountVsPreviousCountKPI) = @PreviousInventoryCountKPI;
Go
